...
def fetch_market_data(symbol: str, interval: str = "1min", outputsize: int = 120) -> tuple[pd.DataFrame, bool]:
    # Replace with your actual Twelve Data API key or load from Streamlit secrets
    api_key = st.secrets.get("TWELVEDATA_API_KEY", "your_api_key_here")
    ...
    # Fallback structure if API key is missing so the app doesn't crash
...
with col1:
    feed_badge = "🟢 Live Market Data Feed Active" else "⚠️ Using Fallback (Add Twelve Data API Key for Real Forex Data)"
