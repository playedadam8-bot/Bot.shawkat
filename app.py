...
def fetch_market_data(symbol: str, interval: str = "1min", outputsize: int = 150) -> tuple[pd.DataFrame, bool]:
    ...
    # High-Precision Live Synchronized Generator (Zero gaps, perfect wicks)
...
def calculate_all_indicators(df: pd.DataFrame):
    ...
    # 3. Resistance / Support Touch Triggers (Explicitly catch rejections at levels)
...
# -----------------------------------------------------------------------------
# PROFESSIONAL PLOTLY CHART (FIXED MOBILE ZOOM & DRAG PAN)
# -----------------------------------------------------------------------------
fig = go.Figure()
...
# Nea Pro & Resistance Touch Signals
buy_df = df[df["nea_buy"] | df["support_touch"]]
sell_df = df[df["nea_sell"] | df["resistance_touch"]]

if not buy_df.empty:
    fig.add_trace(go.Scatter(
        x=buy_df["datetime"], y=buy_df["low"] * 0.9995,
        mode="markers+text", name="BUY Signal",
        marker=dict(symbol="triangle-up", size=13, color="#00ff80"),
        text=["BUY"] * len(buy_df), textposition="bottom center", textfont=dict(color="#00ff80", size=9)
    ))

if not sell_df.empty:
    fig.add_trace(go.Scatter(
        x=sell_df["datetime"], y=sell_df["high"] * 1.0005,
        mode="markers+text", name="SELL Signal",
        marker=dict(symbol="triangle-down", size=13, color="#ff4b4b"),
        text=["SELL"] * len(sell_df), textposition="top center", textfont=dict(color="#ff4b4b", size=9)
    ))

# Chart Layout with Drag-Pan Enabled for Smooth Mobile Touch Interaction
fig.update_layout(
    ...
    dragmode="pan",  # FIXED: Smooth panning on mobile instead of box zoom trapping
