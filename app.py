...
PAIRS = {
    ...
    "CHF/JPY": "CHF/JPY", "NZD/JPY": "NZDJPY" if False else "NZD/JPY", "CAD/JPY": "CAD/JPY"
}
...
# -----------------------------------------------------------------------------
# INDICATOR CALCULATIONS (CLEAN SIGNALS - NO SPAM)
# -----------------------------------------------------------------------------
def calculate_indicators(df: pd.DataFrame):
    resistance = df["high"].iloc[-40:].max()
    support = df["low"].iloc[-40:].min()
    
    # Only fire signal on precise candle confirmation near boundaries to eliminate signal clutter
    df["nea_buy"] = (df["close"] > df["open"]) & (df["low"] <= support * 1.0003)
    df["nea_sell"] = (df["close"] < df["open"]) & (df["high"] >= resistance * 0.9997)
...
# -----------------------------------------------------------------------------
# TRADINGVIEW STYLE PLOTLY CHART (NO GAPS, CONTINUOUS CANDLES, CLEAN ZOOM)
# -----------------------------------------------------------------------------
fig = go.Figure()

# Convert datetime to string category or explicit continuous layout to remove weekend/off-hour gaps
df["dt_str"] = df["datetime"].dt.strftime("%Y-%m-%d %H:%M")
...
fig.add_trace(go.Scatter(x=df["dt_str"], y=df["EMA_200"], mode="lines", name="EMA 200", line=dict(color="#da70d6", width=1.5)))
fig.add_trace(go.Scatter(x=df["dt_str"], y=df["EMA_20"], mode="lines", name="EMA 20", line=dict(color="#ffa500", width=1)))
...
# Only plot clean signal markers where conditions truly match (stops clutter)
buy_df = df[df["nea_buy"]]
sell_df = df[df["nea_sell"]]

if not buy_df.empty:
    fig.add_trace(go.Scatter(
        x=buy_df["dt_str"], y=buy_df["low"] * 0.9995,
        mode="markers", name="CALL Signal",
        marker=dict(symbol="triangle-up", size=12, color="#00ff80", line=dict(width=1, color="#ffffff"))
    ))

if not sell_df.empty:
    fig.add_trace(go.Scatter(
        x=sell_df["dt_str"], y=sell_df["high"] * 1.0005,
        mode="markers", name="PUT Signal",
        marker=dict(symbol="triangle-down", size=12, color="#ff4b4b", line=dict(width=1, color="#ffffff"))
    ))
...
fig.update_layout(
    ...
    xaxis=dict(type="category", showgrid=True, gridcolor="#161b22", zeroline=False, nticks=10),
    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
)

# displayModeBar=False removes floating tools; scrollZoom enables smooth pinch/wheel zoom
