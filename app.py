...
# -----------------------------------------------------------------------------
# COMBINED INDICATOR ENGINE (CLEAN PIVOT SIGNALS & S&R)
# -----------------------------------------------------------------------------
def calculate_all_indicators(df: pd.DataFrame):
    resistance = df["high"].iloc[-35:].max()
    support = df["low"].iloc[-35:].min()
    
    # Filtered Nea Pro / Swing Rejections to avoid spamming every candle
...
# Legend Key Explaining the Clean Signals
st.markdown("""
    <div class="legend-box">
        <b>🧭 Chart Signal & Indicator Legend:</b><br>
        🟢 <span style="color:#00ff80;"><b>CALL 🟢 (Support Reversal)</b></span>: Clean signal at support bounce.<br>
        🔴 <span style="color:#ff4b4b;"><b>PUT 🔴 (Resistance Rejection)</b></span>: Clean signal at resistance rejection.<br>
...
# -----------------------------------------------------------------------------
# PROFESSIONAL PLOTLY CHART (TRADINGVIEW STYLE: NO MODEBAR, 5-DECIMAL PRICES)
# -----------------------------------------------------------------------------
fig = go.Figure()
...
# Filtered Clean Signals (No spamming every candle)
...
# Chart Layout with TradingView-like clean feel (Exact 5-decimal prices, right axis, no toolbar clutter)
fig.update_layout(
    ...
    dragmode="zoom",
    yaxis=dict(side="right", showgrid=True, gridcolor="#161b22", zeroline=False, tickformat=".5f"), # Exact 5-decimal price ticks!
    xaxis=dict(showgrid=True, gridcolor="#161b22", zeroline=False),
    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
)

# displayModeBar=False removes all floating zoom/pan buttons completely like TradingView
